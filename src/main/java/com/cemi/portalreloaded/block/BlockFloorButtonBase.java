...
public abstract class BlockFloorButtonBase extends PortalBlock {

	/** The bounding box for the pressure plate pressed state */
	protected static final AxisAlignedBB PRESSED_AABB = new AxisAlignedBB(-0.9375D, 0.0D, -0.9375D, 0.9375, 0.03125D,
			0.9375D);
	protected static final AxisAlignedBB UNPRESSED_AABB = new AxisAlignedBB(-0.9375D, 0.0D, -0.9375D, 0.9375D, 0.0625D,
			0.9375D);
	/**
	 * This bounding box is used to check for entities in a certain area and then
	 * determine the pressed state.
	 */
	protected static final AxisAlignedBB PRESSURE_AABB = new AxisAlignedBB(-0.9375D, 0.0D, -0.9375D, 0.9375D, 0.25D,
			0.9375D);

	public BlockFloorButtonBase(Material material, String name) {
	...
	public void onBlockAdded(World worldIn, BlockPos pos, IBlockState state) {
		if (worldIn.isAirBlock(pos.west()))
			worldIn.setBlockState(pos.west(), PortalBlocks.dummyFloorButton.getDefaultState());
		else
			return;
		if (worldIn.isAirBlock(pos.north()))
			worldIn.setBlockState(pos.north(), PortalBlocks.dummyFloorButton.getDefaultState());
		else
			return;
		if (worldIn.isAirBlock(pos.west().north()))
			worldIn.setBlockState(pos.west().north(), PortalBlocks.dummyFloorButton.getDefaultState());
		else
			return;
	...
	public void breakBlock(World worldIn, BlockPos pos, IBlockState state) {
		...
		worldIn.setBlockToAir(pos.west().north());
