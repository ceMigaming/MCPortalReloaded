...
public class PortalEntityPlacer extends PortalItem {

	List<String> entitiesList;

	public PortalEntityPlacer(String name, List<String> entitiesList) {
		super(name);
		setCreativeTab(PortalTabs.PORTAL_ENTITIES);
		setHasSubtypes(true);
	...
	public EnumActionResult onItemUse(EntityPlayer player, World worldIn, BlockPos pos, EnumHand hand,
		...
		if (worldIn.isRemote) {
			...
			Entity entity = spawnCreature(worldIn, getNamedIdFrom(itemstack), (double) blockpos.getX() + 0.5D,
	...
	public void getSubItems(CreativeTabs tab, NonNullList<ItemStack> items) {
		if (this.isInCreativeTab(tab)) {
			int meta = 0;
			for (String id : entitiesList) {
				ItemStack itemstack = new ItemStack(this, 1, meta++);
				applyEntityIdToItemStack(itemstack, new ResourceLocation(PortalReloaded.MODID, id)); // FIX
																										// POTENTIAL
																										// MEM
																										// LEAK!!!!!!!!!!!!!!!!!
				items.add(itemstack);
	...
	public void registerItemModel() {
		int meta = 0;
		for (String id : entitiesList) {
			PortalReloaded.proxy.registerItemRenderer(this, meta++, id);
		}
	}
}